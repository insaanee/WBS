	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true) {
	            	...
	            	' <br /> <br /> <a type="button" onclick="printOrder('+response.order_id+')" class="btn btn-info"> <i class="glyphicon glyphicon-print"></i> <b style="color:#fff;"> Print </b> </a>'+
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				'<td style="padding-left:20px;">'+
					...
					'<td style="padding-left:20px;">'+
					'<div class="form-group">'+
					'<input type="number" name="quantity1[]" id="quantity1'+count+'" onkeyup="getTotal('+count+')" autocomplete="off" class="form-control" min="1" readonly />'+
					'</div>'+
				'</td>'+
				'<td style="padding-left:20px;">'+
		...
		if(productId == "") {
			...
			$.ajax({
				...
				success:function(response) {
					...
					$("#quantity1"+row).val(response.quantity);
...
function getTotal(row = null) {
	if(row) {
		...
		var qty1 = $("#quantity1"+row).val();
		if(qty > qty1) {
			
			alert('Warning !! Product stock is low');
		$("#quantity"+row).val(0);	
		...
		if(qty < 0) {
			...
			alert('Warning !! Product quantity can not be negative');
		$("#quantity"+row).val(0);	
...
function paidAmount() {
	...
	if( paid1 >= 0 ) {
	    ...
	    $("#paymentStatus").val("1");
           ...
           alert("warning! plase type valid data");	
           $("#paid").val("0") ; 
